...
import { TX_INSPECTORS, TX_METROS, detectMetroFromAddress, TREC_INSPECTOR_LOOKUP } from '../utils/inspectors'

const ACCENT = '#16a34a'

export default function InspectorPanel({ open, onClose, homeAddress }) {
  ...
  const filtered = metro === 'All'
    ? TX_INSPECTORS
    : TX_INSPECTORS.filter((a) => a.metros.includes(metro))
        ...
        <>
          ...
          >
            <div className="flex items-start justify-between px-6 py-5 border-b border-gray-200">
              <div>
                <h2 id="inspector-panel-title" className="text-lg font-bold text-gray-900">
                  Pre-listing inspectors
                </h2>
                <p className="text-xs text-gray-500 mt-1">
                  Texas TREC-licensed inspectors serving FSBO sellers
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="px-6 py-3 bg-amber-50 border-b border-amber-200">
              <p className="text-xs text-amber-800 leading-relaxed">
                🚧 We&apos;re building a vetted directory. The entries below are placeholders for layout. For now, verify any TX inspector on{' '}
                <a href={TREC_INSPECTOR_LOOKUP} target="_blank" rel="noopener noreferrer" className="underline font-semibold">
                  TREC License Lookup ↗
                </a>{' '}
                before hiring.
              </p>
            ...
            <div className="px-6 py-4 border-b border-gray-100">
              <label htmlFor="inspector-metro" className="block text-[11px] font-medium uppercase tracking-wide text-gray-500 mb-1">
                Metro area
              </label>
              <select
                id="inspector-metro"
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
              {filtered.length === 0 ? (
                <p className="text-sm text-gray-500">
                  No inspectors listed for this metro yet. Try TREC&apos;s lookup above.
                </p>
              ) : filtered.map((a) => (
                  ...
                  <div className="mt-3 flex flex-col gap-1.5">
                    ...
                    {a.website && (
                      <a
                        ...
                        style={{ color: ACCENT }}
                      >
                        {a.placeholder ? 'Find via TREC ↗' : 'Visit website ↗'}
                      </a>
                    )}
                  </div>
                </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
              <p className="text-xs text-gray-500 leading-relaxed">
                ...
                <a
                  href={TREC_INSPECTOR_LOOKUP}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline font-semibold"
                  style={{ color: ACCENT }}
                >
                  TREC&apos;s inspector directory ↗
                </a>{' '}
                for licensed inspectors anywhere in Texas.
              </p>
            </div>
          </motion.aside>
        </>
